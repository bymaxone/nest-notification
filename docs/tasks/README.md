...
|---|---|---|---|
| 1 | [`phase-01-foundation-interfaces.md`](./phase-01-foundation-interfaces.md) | 0 / 11 | 🔴 Not started |
...
- **CI green from PR #1** — the four workflows (`ci`/`codeql`/`scorecard`/`release`) are created in **Task 1.1** and every per-PR gate is incremental-safe (jest `passWithNoTests`, coverage on implemented files, empty-`react` build-output integrity, size budgets, `check:no-prisma`). Mutation is a pre-release gate only; `release.yml` is tag-driven. Every PR must leave CI green.
